...
implements ITileEntityProvider {
    ...
    public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
        if (state.getValue((IProperty)HALF) == BlockDoor.EnumDoorHalf.LOWER) {
            IBlockState iblockstate1 = worldIn.getBlockState(pos.up());
            if (iblockstate1.getBlock() == this) {
                state = state.withProperty((IProperty)HINGE, iblockstate1.getValue((IProperty)HINGE)).withProperty((IProperty)POWERED, iblockstate1.getValue((IProperty)POWERED));
            }
        } else {
