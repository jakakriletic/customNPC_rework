...
public class VersionCompatibility {
    ...
    public static void CheckNpcCompatibility(EntityNPCInterface npc, NBTTagCompound compound) {
        ...
        if (npc.npcVersion == 12 && (list = compound.getTagList("StartPos", 3)).tagCount() == 3) {
    ...
    private static void CompatabilityFix(NBTTagCompound compound, NBTTagCompound check) {
        ...
        for (String name : tags) {
            ...
            if (!(nbt instanceof NBTTagCompound) || !(compound.getTag(name) instanceof NBTTagCompound)) continue;
            VersionCompatibility.CompatabilityFix(compound.getCompoundTag(name), (NBTTagCompound)nbt);
        }
    }
}
