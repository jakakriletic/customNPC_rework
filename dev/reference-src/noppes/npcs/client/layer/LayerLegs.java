...
implements LayerPreRender {
    ...
    private void renderTails(float par7) {
        ...
        GlStateManager.translate((float)(config.transX * par7), (float)(config.transY + this.rotationPointY * par7), (float)(config.transZ * par7 + this.rotationPointZ * par7));
        GlStateManager.translate((float)0.0f, (float)0.0f, (float)((config.scaleZ - 1.0f) * 5.0f * par7));
        GlStateManager.scale((float)config.scaleX, (float)config.scaleY, (float)config.scaleZ);
        this.preRender(data);
        if (data.type == 0) {
            if (data.pattern == 1) {
                this.tail.rotationPointX = -0.5f;
                this.tail.rotateAngleY = (float)((double)this.tail.rotateAngleY - 0.2);
                this.tail.render(par7);
                this.tail.rotationPointX += 1.0f;
                this.tail.rotateAngleY = (float)((double)this.tail.rotateAngleY + 0.4);
                this.tail.render(par7);
    ...
    private void renderLegs(float par7) {
        ...
        this.preRender(data);
        if (data.type == 1) {
            GlStateManager.translate((float)0.0f, (float)(config.transY * 2.0f), (float)(config.transZ * par7 + 0.04f));
            GlStateManager.scale((float)config.scaleX, (float)config.scaleY, (float)config.scaleZ);
            this.naga.render(par7);
        } else if (data.type == 2) {
            GlStateManager.translate((double)0.0, (double)((double)(config.transY * 1.76f) - 0.1 * (double)config.scaleY), (double)(config.transZ * par7));
            GlStateManager.scale((float)1.06f, (float)1.06f, (float)1.06f);
            GlStateManager.scale((float)config.scaleX, (float)config.scaleY, (float)config.scaleZ);
            this.spiderLegs.render(par7);
        } else if (data.type == 3) {
            if (config.scaleY >= 1.0f) {
                GlStateManager.translate((float)0.0f, (float)(config.transY * 1.76f), (float)(config.transZ * par7));
            } else {
                GlStateManager.translate((float)0.0f, (float)(config.transY * 1.86f), (float)(config.transZ * par7));
            }
            GlStateManager.scale((float)0.79f, (float)(0.9f - config.scaleY / 10.0f), (float)0.79f);
            GlStateManager.scale((float)config.scaleX, (float)config.scaleY, (float)config.scaleZ);
    ...
    public void rotateTail(float par1, float par2, float par3, float par4, float par5, float par6) {
        ...
        ModelPartConfig config = this.playerdata.getPartConfig(EnumParts.LEG_LEFT);
        float rotateAngleY = MathHelper.cos((float)(par1 * 0.6662f)) * 0.2f * par2;
        float rotateAngleX = MathHelper.sin((float)(par3 * 0.067f)) * 0.05f;
        ...
        if (partTail != null) {
            if (partTail.type == 2) {
                rotateAngleX = (float)((double)rotateAngleX + 0.5);
            }
            if (partTail.type == 0) {
                rotateAngleX += 0.87f;
            }
        ...
        this.fin.rotateAngleX = this.rodent.rotateAngleX = rotateAngleX;
        this.horse.rotateAngleX = this.rodent.rotateAngleX;
        this.squirrel.rotateAngleX = this.rodent.rotateAngleX;
        this.dragon.rotateAngleX = this.rodent.rotateAngleX;
        this.feathers.rotateAngleX = this.rodent.rotateAngleX;
        this.tail.rotateAngleX = this.rodent.rotateAngleX;
        this.fox.rotateAngleX = this.rodent.rotateAngleX;
        this.fin.rotateAngleY = this.rodent.rotateAngleY = rotateAngleY;
        this.horse.rotateAngleY = this.rodent.rotateAngleY;
        this.squirrel.rotateAngleY = this.rodent.rotateAngleY;
        this.dragon.rotateAngleY = this.rodent.rotateAngleY;
        this.feathers.rotateAngleY = this.rodent.rotateAngleY;
        this.tail.rotateAngleY = this.rodent.rotateAngleY;
        this.fox.rotateAngleY = this.rodent.rotateAngleY;
    }
    ...
    public void preRender(EntityCustomNpc player) {
        ...
        ModelPartData data = this.playerdata.getPartData(EnumParts.LEGS);
        this.model.bipedRightLeg.isHidden = data == null || data.type != 0;
        this.model.bipedLeftLeg.isHidden = this.model.bipedRightLeg.isHidden;
    }
}
