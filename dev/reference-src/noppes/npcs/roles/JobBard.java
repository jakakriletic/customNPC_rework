...
implements IJobBard {
    ...
    public void onLivingUpdate() {
        ...
        if (!MusicController.Instance.isPlaying(this.song)) {
            List list2 = this.npc.world.getEntitiesWithinAABB(EntityPlayer.class, this.npc.getEntityBoundingBox().grow((double)this.minRange, (double)(this.minRange / 2), (double)this.minRange));
            ...
            if (this.npc.getDistanceSq((Entity)player) < MusicController.Instance.playingEntity.getDistanceSq((Entity)player)) {
                MusicController.Instance.playingEntity = this.npc;
            }
        } else if (this.hasOffRange && !(list = this.npc.world.getEntitiesWithinAABB(EntityPlayer.class, this.npc.getEntityBoundingBox().grow((double)this.maxRange, (double)(this.maxRange / 2), (double)this.maxRange))).contains(CustomNpcs.proxy.getPlayer())) {
