...
implements ISchematic {
    ...
    public void load(NBTTagCompound compound) {
        ...
        byte[] addId = compound.hasKey("AddBlocks") ? compound.getByteArray("AddBlocks") : new byte[]{};
    ...
    public NBTTagCompound getNBT() {
        ...
        compound.setTag("TileEntities", (NBTBase)this.tileList);
        return compound;
    }
    ...
    public IBlockState getBlockState(int x, int y, int z) {
        ...
        Block b = Block.getBlockById((int)this.blockArray[i]);
        if (b == null) {
            return Blocks.AIR.getDefaultState();
        }
        return b.getStateFromMeta((int)this.blockDataArray[i]);
    }
    ...
    public static Schematic Create(World world, String name, BlockPos pos, short height, short width, short length) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            if (state.getBlock() == Blocks.AIR || state.getBlock() == CustomItems.copy) continue;
            schema.blockArray[i] = (short)Block.REGISTRY.getIDForObject((Object)state.getBlock());
            schema.blockDataArray[i] = (byte)state.getBlock().getMetaFromState(state);
            if (!(state.getBlock() instanceof ITileEntityProvider)) continue;
            ...
            schema.tileList.appendTag((NBTBase)compound);
        }
        return schema;
    }
}
