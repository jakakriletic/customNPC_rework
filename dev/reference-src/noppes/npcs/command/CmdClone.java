...
extends CommandNoppesBase {
    ...
    public void add(MinecraftServer server, ICommandSender sender, String[] args) {
        ...
        for (EntityNPCInterface npc : list) {
            ...
            if (!npc.writeToNBTAtomically(compound = new NBTTagCompound())) {
                return;
            }
    ...
    public void spawn(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        if (args.length > 2) {
            ...
            if (location.contains(",")) {
                ...
                try {
                    pos = CommandBase.parseBlockPos((ICommandSender)sender, (String[])par, (int)0, (boolean)false);
                }
    ...
    public boolean grid(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        if (args.length > 4) {
            ...
            if (location.contains(",")) {
                ...
                try {
                    curpos = CommandBase.parseBlockPos((ICommandSender)sender, (String[])par, (int)0, (boolean)false);
                }
                catch (NumberInvalidException e) {
                    throw new CommandException("Location should be in numbers", new Object[0]);
                }
        ...
        for (int x = 0; x < width; ++x) {
            for (int z = 0; z < height; ++z) {
                ...
                for (int y = 0; y < 10; ++y) {
                    ...
                    if (!b.causesSuffocation() || b2.causesSuffocation()) continue;
                    npcpos = pos;
                    break;
                }
                Entity entity = EntityList.createEntityFromNBT((NBTTagCompound)compound, (World)world);
                entity.setPosition((double)npcpos.getX() + 0.5, (double)(npcpos.getY() + 1), (double)npcpos.getZ() + 0.5);
                if (entity instanceof EntityNPCInterface) {
                    EntityNPCInterface npc = (EntityNPCInterface)entity;
    ...
    public World getWorld(MinecraftServer server, String t) {
        WorldServer[] ws;
        for (WorldServer w : ws = server.worlds) {
            if (w == null || !(w.provider.getDimension() + "").equalsIgnoreCase(t)) continue;
            return w;
        }
        return null;
    }
