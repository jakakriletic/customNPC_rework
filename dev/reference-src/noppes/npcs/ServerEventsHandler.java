...
public class ServerEventsHandler {
    ...
    public void invoke(PlayerInteractEvent.EntityInteract event) {
        ...
        if (item.getItem() == CustomItems.wand && npcInteracted && !isRemote) {
            ...
            if (isRemote) {
                CustomNpcs.proxy.openGui(MathHelper.floor((double)ServerEventsHandler.mounted.posX), MathHelper.floor((double)ServerEventsHandler.mounted.posY), MathHelper.floor((double)ServerEventsHandler.mounted.posZ), EnumGuiType.MobSpawnerMounter, event.getEntityPlayer());
            }
            ...
            if (!isRemote) {
                ...
                MerchantRecipeList merchantrecipelist = Merchant.getRecipes((EntityPlayer)player);
    ...
    private void doQuest(EntityPlayer player, EntityLivingBase entity, boolean all) {
        ...
        for (QuestData data : playerdata.activeQuests.values()) {
            ...
            if (data.quest.type == 4 && all) {
                List list = player.world.getEntitiesWithinAABB(EntityPlayer.class, entity.getEntityBoundingBox().grow(10.0, 10.0, 10.0));
    ...
    public void commandGive(CommandEvent event) {
        ...
        try {
            EntityPlayerMP player = CommandBase.getPlayer((MinecraftServer)event.getSender().getServer(), (ICommandSender)event.getSender(), (String)event.getParameters()[0]);
            player.getServer().futureTaskQueue.add(ListenableFutureTask.create(Executors.callable(() -> ServerEventsHandler.lambda$commandGive$0((EntityPlayer)player))));
        }
    ...
    public void saveChunk(ChunkDataEvent.Save event) {
        for (ClassInheritanceMultiMap map : event.getChunk().getEntityLists()) {
            for (Entity e : map) {
