...
extends EntityAIBase {
    ...
    public void rotate(int degrees) {
        this.forced = true;
        this.npc.rotationYaw = this.npc.renderYawOffset = (float)degrees;
    ...
    public void updateTask() {
        ...
        if (this.forced && this.forcedEntity != null) {
            ...
            while (ita.hasNext()) {
                ...
                double distance = entity.getDistanceSq((Entity)this.npc);
                if (distance < closestDistance) {
                    closestDistance = entity.getDistanceSq((Entity)this.npc);
                    lookat = entity;
                    continue;
                }
            ...
            lookat = this.npc.world.getClosestPlayerToEntity((Entity)this.npc, 16.0);
        }
        if (lookat != null) {
        ...
        if (this.rotatebody) {
            if (this.idle == 0 && this.npc.getRNG().nextFloat() < 0.004f) {
                double var1 = Math.PI * 2 * this.npc.getRNG().nextDouble();
                if (this.npc.ais.getStandingType() == 3) {
                    var1 = Math.PI / 180 * (double)this.npc.ais.orientation + 0.6283185307179586 + 1.8849555921538759 * this.npc.getRNG().nextDouble();
                }
        ...
        if (this.npc.ais.getStandingType() == 1 && !this.forced) {
            this.npc.rotationYaw = this.npc.renderYawOffset = (float)this.npc.ais.orientation;
            this.npc.rotationYawHead = this.npc.renderYawOffset;
        }
    }
}
