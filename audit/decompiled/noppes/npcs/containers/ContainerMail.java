...
extends ContainerNpcInterface {
    ...
    public ContainerMail(EntityPlayer player, boolean canEdit, boolean canSend) {
        ...
        for (j = 0; j < 3; ++j) {
            for (k = 0; k < 9; ++k) {
                this.func_75146_a(new Slot((IInventory)player.field_71071_by, k + j * 9 + 9, 28 + k * 18, 175 + j * 18));
            }
        }
        for (j = 0; j < 9; ++j) {
            this.func_75146_a(new Slot((IInventory)player.field_71071_by, j, 28 + j * 18, 230));
        }
    }
    ...
    public void func_75134_a(EntityPlayer player) {
        ...
        if (!this.canEdit && !player.field_70170_p.field_72995_K) {
